...
{{- $table := (print "` + GetTableName(\"" .Table.TableName "\") + `" ) -}}
{{- if .Comment -}}
// {{ .Comment }}
{{- else -}}
...
// Deled provides information if the {{ .Name }} has been deled from the database.
func ({{ $short }} *{{ .Name }}) Deled() bool {
...
func ({{ $short }} *{{ .Name }}) Delete(db XODB) error {
	...
	{{ if gt ( len .PrimaryKeyFields ) 1 }}
		// sql query with composite primary key
		var sqlstr = `DELETE FROM {{ $table }} WHERE {{ colnamesquery .PrimaryKeyFields " AND " }}`
	...
	{{- else }}
		// sql query
		var sqlstr = `DELETE FROM {{ $table }} WHERE {{ colname .PrimaryKey.Col }} = ?`
